...
void make_EDGES_p_table() {
  ...
  std::cout << "Table saved (" << ((float)(clock() - t)) / CLOCKS_PER_SEC
            << "s)\n\n"
            << std::flush;
}
...
bool prune(Puzzle *p, int depth) {
  uint64_t h;
  if (EDGES_p_table[p->encode_EDGES_p()] > depth)
    return true;
  if (CORNERS_p_table[p->encode_CORNERS_p()] > depth)
    return true;
  if (CORNERS_o_table[p->encode_CORNERS_o()] > depth)
    return true;
  return false;
}
